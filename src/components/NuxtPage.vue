...
import type { KeepAliveProps, TransitionProps, SuspenseProps } from 'vue'
...
const {
  ...
  onResolve,
  ...transitionProps
} = defineProps<TransitionProps & KeepAliveProps & SuspenseProps>()
</script>
  ...
  <RouterView #default="{ Component }">
    <template v-if="Component">
      <Transition v-bind="transitionProps">
        <KeepAlive :include :max :exclude>
          <Suspense :suspensible :timeout :onFallback :onPending :onResolve>
            <component :is="Component" v-bind="$attrs" />
            <template #fallback>
              <slot>
                <div class="m-auto mt-1 loader" />
              </slot>
            </template>
          </Suspense>
        </KeepAlive>
...
.loader {
  ...
  background: linear-gradient(to right, #000 0%, #000 0%) 0 0/100% 100% content-box padding-box
    no-repeat;
  position: relative;
  overflow: hidden;
...
.loader::after {
  ...
  left: calc(50% - 16px);
  background:
    linear-gradient(90deg, #0000 12px, #f92033 0 22px, #0000 0 26px, #fdc98d 0 32px, #0000) bottom
      26px left 50%,
    ...
    linear-gradient(
      ...
      bottom 8px left 50%,
    linear-gradient(90deg, #0000 12px, #222a87 0 18px, #feee49 0 20px, #222a87 0 30px, #643700 0)
      bottom 6px left 50%,
